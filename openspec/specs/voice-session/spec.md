...
The session SHALL connect to Gemini Live using the model from `FRIDAY_MODEL` (default `gemini-3.8-live`), the prebuilt voice from `FRIDAY_VOICE` (default `Aoede`), audio-only response modality, input and output audio transcription enabled, the Friday system prompt, and the declarations of every registered tool.

#### Scenario: Session opens
- **WHEN** a transport opens a session
- **THEN** a Gemini Live connection is established with the configured model, voice, system prompt and all tool declarations
...
When the model calls `end_conversation`, the session SHALL close after the current turn completes so the model's final words are delivered, and the `closed` event SHALL carry `ended: <reason>`.
